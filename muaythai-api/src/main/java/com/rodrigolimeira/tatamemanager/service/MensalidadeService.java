...
public class MensalidadeService {
    ...
    // Configurado para rodar todo dia às 09:00h, junto com a checagem de mensalidade.
    // Roda separado da verificarAtrasos() de propósito: um envio não deve travar o outro
    // caso um dos dois falhe.
    @Scheduled(cron = "0 0 9 * * ?")
    public void verificarAniversariantes() {
        LocalDate hoje = LocalDate.now();
        List<Aluno> aniversariantes = alunoRepository.findAniversariantesAtivos(
                hoje.getMonthValue(), hoje.getDayOfMonth());

        System.out.println("Verificando aniversariantes de hoje (" + hoje + ")... Encontrados: "
                + aniversariantes.size());

        for (Aluno aluno : aniversariantes) {
            enviarMensagemAniversario(aluno);
        }
    }

    // Pacote-privado: mesma lógica de tentativas do enviarMensagemWhatsApp, mas sem
    // gravar nada no banco (não há necessidade de anti-spam aqui, já que o método só
    // roda uma vez por dia e cada aluno só faz aniversário uma vez por ano).
    boolean enviarMensagemAniversario(Aluno aluno) {
        String mensagem = String.format(
                "Sawadee Krap %s! 🥊🎉 A equipe de Muay Thai deseja um Feliz Aniversário! " +
                        "Que venham muitos socos, chutes e vitórias nesse novo ano de vida. Oss!",
        ...
        for (int tentativa = 1; tentativa <= MAX_TENTATIVAS; tentativa++) {
            try {
                ...
                System.out.println("🎂 PARABÉNS ENVIADO: " + aluno.getNomeCompleto());
                return true;

            } catch (Exception e) {
                System.out.println("❌ Tentativa " + tentativa + "/" + MAX_TENTATIVAS +
                        " falhou ao enviar parabéns para " + aluno.getNomeCompleto() + ": " + e.getMessage());
            }
        }
    ...
    boolean enviarMensagemWhatsApp(Aluno aluno) {
        String mensagem = String.format(
                "Sawadee Krap %s! Tudo bem? Passando para lembrar da mensalidade da nossa equipe de Muay Thai neste mês.",
                aluno.getNomeCompleto());
